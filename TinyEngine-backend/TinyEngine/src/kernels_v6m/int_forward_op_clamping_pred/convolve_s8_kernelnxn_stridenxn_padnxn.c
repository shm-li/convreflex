...
tinyengine_status convolve_s8_kernelnxn_stridenxn_padnxn_clamping_pred(
	...
	for (int i_out_y = 0; i_out_y < output_y; i_out_y++) {
		...
		// printf("Writing to dst_end %d\r\n", dst - input_16b_buffer);
		// fflush(stdout);
		int elem_cnt = elem_per_row * pad_top;
		while (elem_cnt > 0) { //total: 16bit * input_ch * kernel_w * pad_top
			// printf("Writing to dst[%d]/%d\r\n", dst - input_16b_buffer, elem_per_row * pad_top);
			// fflush(stdout);
			*dst++ = pad_out;
			elem_cnt--;
		}
		...
		for (int i_out_x = 0; i_out_x < output_x; i_out_x++) {
			...
			// printf("out[%d, %d], pad: %d, %d, %d, %d\r\n", i_out_y, i_out_x, pad_top, pad_bottom, pad_left, pad_right);
			// fflush(stdout);
			...
			out = mat_mult_kernel_s8_s16_one_column_clamping_pred(kernel, runtime_buf,
					termination_steps, 
					termination_bounds,
					...
					bias, out);
#else
			out = mat_mult_kernel_s8_s16_one_column_clamping_pred_with_profile(kernel, runtime_buf,
					termination_steps, 
					termination_bounds,
					input_offset,
					output_ch, output_shift, output_mult, output_offset,
