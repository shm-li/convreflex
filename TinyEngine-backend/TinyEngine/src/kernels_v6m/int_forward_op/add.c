...
tinyengine_status add(int size, ADD_params* params, const int8_t* input1_data,
  ...
  // printf("ADD PARAMS: offset %d, %d, %d, "
  //   "left_shift %d, "
  //   "shift %d, %d, %d, "
  //   "multiplier %d, %d, %d, "
  //   "out min/max %d, %d\n",
  ...
  //   params->input1_multiplier, params->input2_multiplier, params->output_multiplier,
  ...
  for (int i = 0; i < size; ++i) {
    ...
    // printf("%d (%d) + %d (%d) = %d (%d)\n", input1_data[i], input1_val, input2_data[i], input2_val, raw_output, clamped_output);
  }
  // for (int i = 0; i < size; ++i) {
  //   printf("%d: %d\n", i, output_data[i]);
  // }
  ...
  for (int h = 0; h < input_h; ++h) {
    for (int w = 0; w < input_w; ++w) {
      for (int c = 0; c < input_c; ++c) {
        const int32_t input1_val = params->input1_offset + input1_data[(h * input_w + w) * input_c + c];
        const int32_t input2_val = params->input2_offset + input2_data[c];
        const int32_t shifted_input1_val = input1_val * (1 << params->left_shift);
        const int32_t shifted_input2_val = input2_val * (1 << params->left_shift);
        const int32_t scaled_input1_val =
            MultiplyByQuantizedMultiplierSmallerThanOneExp(
